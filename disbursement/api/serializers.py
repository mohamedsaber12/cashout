...
class SingleStepserializer(serializers.Serializer):
    ...
    creditor_account_number = serializers.CharField(
        max_length=100,
        required=False
    )
    ...
    creditor_bank  = serializers.CharField(
        max_length=100,
        required=False
    )
    ...
    def validate(self, attr):
        ...
        if issuer == 'bank_card':
            ...
            if not attr.get("transaction_type") in ["CASH_TRANSFER", "SALARY", "PREPAID_CARD", "CREDIT_CARD"] :
                raise serializers.ValidationError(
                    _(f"transaction_type must be one of this ( CASH_TRANSFER, SALARY, PREPAID_CARD, CREDIT_CARD ) ")
                )

            if not attr.get('creditor_bank'):
                raise serializers.ValidationError(
                    _(f"creditor_bank {is_required_msg}")
                )
            ...
            if not attr.get('first_name'):
                 raise serializers.ValidationError(
                    _(f"first_name {is_required_msg}")
                 )
            if not attr.get('last_name'):
                 raise serializers.ValidationError(
                    _(f"last_name {is_required_msg}")
                 )
            if not attr.get('email'):
                 raise serializers.ValidationError(
                    _(f"email {is_required_msg}")
                )
