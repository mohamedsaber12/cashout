...
from utilities.admin import LogEntryAdmin, BudgetAdmin
...
BUDGET_READ_ONLY_FIELDS_WITH_SUPERUSER = [
    'total_disbursed_amount', 'updated_at', 'created_at', 'created_by', 'current_balance'
]
BUDGET_READ_ONLY_FIELDS_WITHOUT_SUPERUSER = [
    'total_disbursed_amount', 'updated_at', 'created_at', 'created_by', 'current_balance',
    'disburser', 'current_balance', 'total_disbursed_amount', 'updated_at'
]
...
class LogEntryAdminTests(TestCase):
    def setUp(self):
        self.logEntryAdmin = LogEntryAdmin(
                model=LogEntry, admin_site=AdminSite()
        )
    ...
    def test_has_delete_permission(self):
        self.assertEqual(self.logEntryAdmin.has_delete_permission(CurrentRequest), False)

    # check if log entry admin model has change permission
    def test_has_change_permission(self):
        self.assertEqual(self.logEntryAdmin.has_change_permission(CurrentRequest), False)


class BudgetAdminTests(TestCase):
    def setUp(self):
        self.budgetAdmin = BudgetAdmin(
                model=Budget, admin_site=AdminSite()
        )
    ...
    def test_get_readonly_fields_with_superuser(self):
        self.assertEqual(self.budgetAdmin.get_readonly_fields(
            CurrentRequest(user=self.superuser)),
            BUDGET_READ_ONLY_FIELDS_WITH_SUPERUSER
        )
    # test read only fields
    ...
    # check if budget admin model has add permission
    def test_has_add_permission(self):
        self.assertEqual(self.budgetAdmin.has_add_permission(
            CurrentRequest(user=self.superuser)), True
        )
