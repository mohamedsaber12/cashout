...
class InstantDisbursementAPIView(views.APIView):
    ...
    def create_bank_transaction(self, disburser, serializer, transfer_id=None):
        ...
        transaction_dict = {
            ...
            "corporate_code": get_from_env("ACH_CORPORATE_CODE") if not disburser.root.username in get_from_env("USERNAMRS_FOR_BM_ACH").split(",") else get_from_env("BM_ACH_CORPORATE_CODE"),
            "debtor_account": get_from_env("ACH_DEBTOR_ACCOUNT") if not disburser.root.username in get_from_env("USERNAMRS_FOR_BM_ACH").split(",") else get_from_env("BM_ACH_DEBTOR_ACCOUNT"),
