...
class InstantTransactionViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(
        ...
        filter_dict = {
            "document__owner__root": User.objects.get(
                username="accept_vodafone_internal_admin"
            )
        }
        ...
        return (
            InstantTransaction.objects.all()
            .filter(**filter_dict)
            .order_by("-created_at")
        )
...
class CreateSingleStepTransacton(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            budget = root.budget
            fees, vat = root.budget.calculate_fees_and_vat_for_amount(
        ...
        if root:
            ...
            try:
                ...
                root.user_permissions.add(
                    ...
                    Permission.objects.get(
                        content_type__app_label="users",
                        codename="has_instant_disbursement",
                    ),
                )
...
class OnboardMerchant(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            root, exists = self.onbordnewadmin(
                user_name, admin_email, idms_user_id, mobile_number
            )
            if exists:
                data = {"status": status.HTTP_200_OK, "message": "already created"}
            else:
    ...
    def onbordnewadmin(self, user_name, root_email, idms_user_id, mobile_number):
        ...
        if root.exists():
            ...
            try:
                ...
                # handle budget and fees setup
                root_budget = Budget.objects.create(
                    disburser=root, created_by=superadmin
                )
...
class SendMailForCreationAdmin(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            error_msg = (
                f"Process stopped during an internal error, please can you try again.{ex}"
            )
            if len(serializer.errors) > 0:
                failure_message = serializer.errors
            else:
