...
class BulkDisbursementThroughOneStepCashin(Task):
    ...
    def create_bank_transaction_from_instant_transaction(self, checker, record):
        ...
        transaction_dict = {
            ...
            "category_code": "MOBI",
            "purpose": "CASH",
            "corporate_code": get_from_env("ACH_CORPORATE_CODE") if not checker.root.username in get_from_env("USERNAMRS_FOR_BM_ACH").split(",") else get_from_env("BM_ACH_CORPORATE_CODE"),
            "debtor_account": get_from_env("ACH_DEBTOR_ACCOUNT") if not checker.root.username in get_from_env("USERNAMRS_FOR_BM_ACH").split(",") else get_from_env("BM_ACH_DEBTOR_ACCOUNT"),
