...
class BalanceInquiryMockResponse:
    ...
    def __init__(self):
        ...
        self.ok=True
    def json(self):
        return {'TXNSTATUS':'200','HAS_PIN':False, 'USER_TYPE':'Agent','BALANCE':2000}
...
class MockResponse:
    ...
    def json(self):
        return {
            "TRANSACTIONS":[{'TXNSTATUS':'1','HAS_PIN':False, 'USER_TYPE':'Agent',}],
        }
...
class MockResponseWithError:
    ...
    def json(self):
        return {
            "TRANSACTIONS":[{'TXNSTATUS':'1','HAS_PIN':True, 'USER_TYPE':'test',}],
        }
...
class MockResponseWithUSER_TYPE_Agent:
    ...
    def __init__(self):
        ...
        self.ok=True
    def json(self):
        return {"TRANSACTIONS":[{"USER_TYPE":"false","WALLET_STATUS":"Active","USER_PROFILE":"Vodafone Agent","MSISDN":"00201020045450","MESSAGE":"inquire wallet status","HAS_PIN":False}]}

def MockValidate_Agent_Wallet():
    transactions=[{"USER_TYPE":False,"WALLET_STATUS":"Active","USER_PROFILE":"3253256t2365t2","MSISDN":"00201021469733","MESSAGE":"inquire wallet status","HAS_PIN":False}]
...
class SuperAdminAgentsSetupTests(TestCase):
    ...
    def setUp(self):
        ...
        CallWalletsModerator.objects.create(
            ...
            user_inquiry=True, balance_inquiry=False
        )
    ...
    @patch("disbursement.views.SuperAdminAgentsSetup.validate_agent_wallet", return_value=MockValidate_Agent_Wallet())
    def test_post_method_with_ClientModel_P2M_(self, mocked):
        self.super_agent = Agent(msisdn='01021469732', wallet_provider=self.root, super=True)
    ...
    def test_post_method_with_ClientModel_EXISTING_SUPERAGENT_NEW_AGENTS(self, mocked):
        ...
        self.agent = Agent(msisdn='01021469732', wallet_provider=self.root)
        self.agent.save()
        self.client_user.agents_onboarding_choice=ClientModel.EXISTING_SUPERAGENT_NEW_AGENTS
        self.client_user.save()
        self.client.force_login(self.super_admin)
        token, created = ExpiringToken.objects.get_or_create(user=self.root)
        if created:
    ...
    def test_post_method_with_ClientModel_EXISTING_SUPERAGENT_NEW_AGENTS_without_transaction(self, mocked):
        self.super_agent = Agent(msisdn='01021469732', wallet_provider=self.root, super=True)
        self.super_agent.save()
        self.agent = Agent(msisdn='01021469732', wallet_provider=self.root)
        self.agent.save()
    ...
    def test_post_method_with_ClientModel_EXISTING_SUPERAGENT_NEW_AGENTS_with_user_type_false(self, mocked):
        self.super_agent = Agent(msisdn='01021469732', wallet_provider=self.root, super=True)
        self.super_agent.save()
        self.agent = Agent(msisdn='01021469732', wallet_provider=self.root)
        self.agent.save()
...
class SingleStepTransactionMockResponse_failed:
    ...
    def __init__(self):
        ...
        self.ok=True
    def json(self):
        return {'TXNSTATUS':'200','disbursement_status':'failed','success':'success'}
...
class SingleStepTransactionsViewTests(TestCase):
    ...
    def setUp(self):
        ...
        self.support_user=SupportUser.objects.create(username="support_user1",email="support_user1@gmail.com",user_type=8, id=151)
        self.support_user.user_permissions. \
            add(Permission.objects.get(
                content_type__app_label='users', codename='accept_vodafone_onboarding')
        )
    ...
    def test_view_url_accessible_by_name(self):
        ...
        response = self.client.get(
            '%s?admin_hierarchy=1&issuer=wallets' %(reverse('disbursement:single_step_list_create'))
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_post_method_on_vodafone_using_user_from_accept_with_failed_request(self, mocked):
        self.root.from_accept=True
        self.root.save()
        fees_setup_vodafone = FeeSetup(budget_related=self.budget, issuer='vf',
                                       fee_type='p', percentage_value=2.25)
    ...
    def test_post_method_on_vodafone_using_user_from_accept_with_successful_request(self, mocked):
        self.root.from_accept=True
        self.root.save()
        fees_setup_vodafone = FeeSetup(budget_related=self.budget, issuer='vf',
                                       fee_type='p', percentage_value=2.25)
    ...
    @patch("requests.post", return_value=SingleStepTransactionMockResponse_Successful())
    def test_post_method_on_vodafone_using_user_from_accept_and_allowed_to_be_bulk_with_successful_request(self, mocked):
        self.root.from_accept=True
...
class DisbursementTests(TestCase):
    ...
    @patch("requests.post", return_value=DisbursementMockResponse())
    def test_disburse_document_using_user_hierarchy_not_equal_owner_hierarchy(self, mocked):
        self.client.force_login(self.checker_user)
...
class DisbursementDocTransactionsViewTests(TestCase):
    ...
    def test_sheet_transactions_view_with_ajax_with_search(self):
        self.client.force_login(self.root)
        response = self.client.get(
            '%s?search=test' % (reverse('disbursement:disbursed_data',
                kwargs={'doc_id':self.doc.id}
            )), {}, HTTP_X_REQUESTED_WITH='XMLHttpRequest'
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_bank_wallets_sheet_transactions_view(self):
        ...
        self.doc.save()
        self.instanttransaction1= InstantTransaction.objects.create(document=self.doc, from_user=self.checker_user, issuer_type=InstantTransaction.AMAN, disbursed_date=datetime.datetime.now(), status='d')
        self.instanttransaction2= InstantTransaction.objects.create(document=self.doc, from_user=self.checker_user, issuer_type=InstantTransaction.AMAN, disbursed_date=datetime.datetime.now(), status='P')
    ...
    def test_bank_wallets_sheet_transactions_view_with_pending_transaction(self):
        ...
        self.doc.save()
        self.instanttransaction1= InstantTransaction.objects.create(document=self.doc, from_user=self.checker_user, issuer_type=InstantTransaction.AMAN, disbursed_date=datetime.datetime.now(), status='d')
        self.instanttransaction2= InstantTransaction.objects.create(document=self.doc, from_user=self.checker_user, issuer_type=InstantTransaction.AMAN, disbursed_date=datetime.datetime.now(), status='F')
        self.instanttransaction2= InstantTransaction.objects.create(document=self.doc, from_user=self.checker_user, issuer_type=InstantTransaction.AMAN, disbursed_date=datetime.datetime.now(), status='R')
        self.instanttransaction3= InstantTransaction.objects.create(document=self.doc, from_user=self.checker_user, issuer_type=InstantTransaction.AMAN, disbursed_date=datetime.datetime.now(), status='J')
...
class FailedDisbursedForDownloadTests(TestCase):
    ...
    def test_file_not_exist(self):
        self.client.force_login(self.root)
        response = self.client.get(f'/disburse/export_failed_download/{self.doc.id}/?filename=test')
        self.assertEqual(response.status_code, 404)
    ...
    def test_file_exist(self):
        with open('/app/mediafiles/media/documents/disbursement/test.txt', 'w') as fp:
            pass
        self.client.force_login(self.root)
        response = self.client.get(f'/disburse/export_failed_download/{self.doc.id}/?filename=test.txt')
        self.assertEqual(response.status_code, 200)
...
class DownloadFailedValidationFileTests(TestCase):
    ...
    def test_file_not_exist(self):
        ...
        response = self.client.get(f'/disburse/export_failed_validation_download/{self.doc.id}/?filename=test')
        self.assertEqual(response.status_code, 404)

    def test_file_without_filename(self):
        self.client.force_login(self.maker_user)
        response = self.client.get(f'/disburse/export_failed_validation_download/{self.doc.id}/')
        self.assertEqual(response.status_code, 404)
    ...
    def test_file_exist(self):
        with open('/app/mediafiles/media/documents/disbursement/test.txt', 'w') as fp:
            pass
        self.client.force_login(self.maker_user)
        response = self.client.get(f'/disburse/export_failed_validation_download/{self.doc.id}/?filename=test.txt')
        self.assertEqual(response.status_code, 200)
...
class download_exported_transactionsTests(TestCase):
    ...
    def test_redirect_if_not_logged_in(self):
        response = self.client.get(
            reverse('disbursement:download_exported'
            )
        )
        self.assertRedirects(response, '/user/login/')
    ...
    def test_file_not_exist(self):
        self.client.force_login(self.maker_user)
        response = self.client.get(f'/disburse/download_exported_transactions/?filename=test')
        self.assertEqual(response.status_code, 404)
    ...
    def test_file_exist(self):
        with open('/app/mediafiles/media/documents/disbursement/test.txt', 'w') as fp:
            pass
        self.client.force_login(self.maker_user)
        response = self.client.get(f'/disburse/download_exported_transactions/?filename=test.txt')
        self.assertEqual(response.status_code, 200)
...
class HomeViewTest(TestCase):
    def setUp(self):
        ...
        self.request = RequestFactory()
        self.client = Client()
        self.makeruser=MakerUser.objects.create(username="makerUser1", root=self.root,user_type=1,
            email='user1112@gmail.com')
        self.checkeruser=CheckerUser.objects.create(username="checkerUser1", root=self.root,user_type=2,
            email='user41112@gmail.com')
        self.support_user=SupportUser.objects.create(username="support_user",email="support_user@gmail.com",user_type=8)
        self.support_user_setup=SupportSetup.objects.create(support_user=self.support_user, user_created=self.super_admin)

        
        
        self.instantapiviewer=InstantAPIViewerUser.objects.create(username="InstantAPIViewerUser1", root=self.root,user_type=7,
            email='user112@gmail.com')
        self.instantapichecker=InstantAPICheckerUser.objects.create(username="InstantAPIcheckerUser1", root=self.root,user_type=7,
            email='userd112@gmail.com')
        self.instantapiviewer.user_permissions. \
            add(Permission.objects.get(
                content_type__app_label='users', codename='instant_model_onboarding')
        )
        self.instanttransaction= InstantTransaction.objects.create(from_user=self.instantapichecker)
        ...
        self.instanttransaction1= InstantTransaction.objects.create(from_user=self.instantapichecker,issuer_type=InstantTransaction.ETISALAT,disbursed_date=datetime.datetime.now())
        self.instanttransaction1= InstantTransaction.objects.create(from_user=self.instantapichecker,issuer_type=InstantTransaction.AMAN,disbursed_date=datetime.datetime.now())
        self.instanttransaction1= InstantTransaction.objects.create(from_user=self.instantapichecker,issuer_type=InstantTransaction.ORANGE,disbursed_date=datetime.datetime.now())
        self.instanttransaction1= InstantTransaction.objects.create(from_user=self.instantapichecker,issuer_type=InstantTransaction.BANK_WALLET,disbursed_date=datetime.datetime.now())
    ...
    def test_view_uses_query_parameter(self):
        self.client.force_login(self.instantapiviewer)
        response = self.client.get('%s?export_issuer=wallets&export_start_date=2022-1-09&export_end_date=2022-1-11'%(reverse('disbursement:home_root')))
        self.assertEqual(response.status_code, 302)

    def test_view_with_export_issuer_banks(self):
        self.client.force_login(self.instantapiviewer)
        response = self.client.get('%s?export_issuer=banks&export_start_date=2022-1-09&export_end_date=2022-1-11'%(reverse('disbursement:home_root')))
        self.assertEqual(response.status_code, 302)
    def test_view_with_export_issuer_vodafone_etisalat_aman(self):
        self.client.force_login(self.instantapiviewer)
        self.export_issuer='vodafone/etisalat/aman'
        response = self.client.get(f'%s?export_issuer={self.export_issuer}&export_start_date=2022-1-09&export_end_date=2022-1-11'%(reverse('disbursement:home_root')))
        self.assertEqual(response.status_code, 302)
...
class DisbursementDataListViewTests(TestCase):
    ...
    def setUp(self):
        ...
        self.request = RequestFactory()
        self.client = Client()
        self.makeruser=MakerUser.objects.create(username="makerUser11", root=self.root,user_type=1,
            email='user11712@gmail.com')
        self.checkeruser=CheckerUser.objects.create(username="checkerUser11", root=self.root,user_type=2,
            email='user411712@gmail.com')
        self.makeruser.user_permissions. \
            add(Permission.objects.get(
                content_type__app_label='users', codename='accept_vodafone_onboarding'
            )
        )
        self.checkeruser.user_permissions. \
            add(Permission.objects.get(
                content_type__app_label='users', codename='accept_vodafone_onboarding'
            )
        )
    ...
    def test_view_url_accessible_by_name_for_checker(self):
        self.client.force_login(self.checkeruser)
        response = self.client.get(
            '%s?number=1&issuer=vodafone&start_date=2022-10-1&end_date=2022-11-1' % reverse('disbursement:vf_et_aman_list')
        )
        self.assertEqual(response.status_code, 200)
...
class OrangeBankWalletListViewTests(TestCase):
    ...
    def test_view_uses_correct_template(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'disbursement/orange_bank_wallet_trx_list.html')
    ...
    def test_view_url_accessible_by_name_for_checker(self):
        self.client.force_login(self.checkeruser)
        response = self.client.get(
            '%s?number=1&issuer=vodafone&start_date=2022-10-1&end_date=2022-11-1' % reverse('disbursement:orange_bank_wallet_list')
        )
        self.assertEqual(response.status_code, 200)
...
class BanksListViewTests(TestCase):
    ...
    def test_view_url_accessible_by_name_for_checker(self):
        self.client.force_login(self.checkeruser)
        response = self.client.get(
            '%s?number=1&account_number=114&start_date=2022-10-1&end_date=2022-11-1' % reverse('disbursement:banks_list')
        )
        self.assertEqual(response.status_code, 200)
