...
class BankTransactionsChannel:
    ...
    def accumulate_send_transaction_payload(trx_obj, amount_to_be_deducted=0):
        ...
        payload["TransactionDateTime"] = trx_obj.created_at.strftime(
            "%d/%m/%Y %H:%M:%S"
        )
        payload["CategoryCode"] = trx_obj.category_code
        payload["TransactionPurpose"] = trx_obj.purpose
        payload["TransactionAmount"] = float(
            decimal.Decimal(trx_obj.amount) - decimal.Decimal(amount_to_be_deducted)
        )
    ...
    def post(url, payload, bank_trx_obj):
        ...
        ACH_SEND_TRX_LOGGER.debug(
            _(f"[request] [{log_header}] [{bank_trx_obj.user_created}] -- {payload}")
        )
        ...
        try:
            ...
            ACH_SEND_TRX_LOGGER.debug(
                _(
                    f"[response] [{log_header}] [{bank_trx_obj.user_created}] -- {response_log_message}"
                )
            )
    ...
    def map_response_code_and_message(
        ...
        if instant_trx_obj:
            issuer = (
                "orange"
                if instant_trx_obj.issuer_type == AbstractBaseIssuer.ORANGE
                else "bank_wallet"
            )
        else:
            issuer = "bank_card"
        ...
        if response_code in ["8000", "8111"]:
            ...
            bank_trx_obj.save()
            ACH_SEND_TRX_LOGGER.debug(
                f"[message] obj balance before {bank_trx_obj.balance_before} obj balance after {bank_trx_obj.balance_after}"
            )
            ...
            instant_trx_obj.mark_pending(
                response_code, instant_message
    ...
    def update_bank_trx_status(bank_trx_obj, json_response):
        ...
        # 3. Otherwise, start creating new bank transaction with the new status code
            ...
            if response_code == "8111":
                ...
                instant_trx.mark_pending(
                    response_code, INSTANT_TRX_BEING_PROCESSED
                ) if instant_trx else None

            # 3.2) Transaction is accepted by the bank - first settlement or final settlement
            elif response_code in ["8222", "8333"]:
                bank_trx_message = (
                    BANK_TRX_IS_SUCCESSFUL_1
                    if response_code == "8222"
                    else BANK_TRX_IS_SUCCESSFUL_2
                )
                ...
                instant_trx.mark_successful(
                    response_code, INSTANT_TRX_IS_ACCEPTED
                ) if instant_trx else None

            # 3.4) Handle bank reject and return cases
            elif (
                response_code in TRX_REJECTED_BY_BANK_CODES + TRX_RETURNED_BY_BANK_CODES
            ):
                ...
                instant_trx.mark_failed(
                    "8888", INSTANT_TRX_IS_REJECTED
                ) if instant_trx else None
                balance_before = (
                    new_trx_obj.user_created.root.budget.get_current_balance()
                )
            ...
            if (
                response_code
                and response_description
                and bank_trx_obj.user_created.root.root.callback_url
            ):
                callback_url = bank_trx_obj.user_created.root.callback_url
                callback_payload = {
                    "transaction_id": str(
                        new_trx_obj.parent_transaction.transaction_id
                    ),
                    "issuer": "bank_card",
                    ...
                    "status_description": str(
                        new_trx_obj.transaction_status_description
                    ),
                    "client_transaction_reference": str(
                        new_trx_obj.client_transaction_reference
                    ),
                    "created_at": new_trx_obj.parent_transaction.created_at.strftime(
                        "%Y-%m-%d %H:%M:%S.%f"
                    ),
                    "updated_at": new_trx_obj.updated_at.strftime(
                        "%Y-%m-%d %H:%M:%S.%f"
                    ),
                }
                ...
                CALLBACK_REQUESTS_LOGGER.debug(
                    _(
                        f"[callback request] [{log_header}] [{callback_url}] [{bank_trx_obj.user_created}] -- {callback_payload}"
                    )
                )
    ...
    def send_transaction(bank_trx_obj, instant_trx_obj, balance_before=0):
        ...
        try:
            ...
            if bank_trx_obj.user_created.root.username == "UVA-Admin":
                remaining_amounts = RemainingAmounts.objects.filter(
                    remaining_amount__gt=0
                )
                for remaining_amount_obj in remaining_amounts:
                    if (
                        remaining_amount_obj.mobile
                        in bank_trx_obj.creditor_account_number
                    ):
                        if (
                            decimal.Decimal(bank_trx_obj.amount)
                            - remaining_amount_obj.remaining_amount
                            >= 1
                        ):
                            ...
                            amount_to_be_deducted = decimal.Decimal(
                                bank_trx_obj.amount
                            ) - decimal.Decimal(1)
                            remaining_amount_obj.remaining_amount = (
                                remaining_amount_obj.remaining_amount
    ...
    def get_transaction_status(bank_trx_obj):
        ...
        try:
            ...
            response = BankTransactionsChannel.get(
                api_url, payload, bank_trx_obj
            )
            new_bank_trx_obj = BankTransactionsChannel.update_bank_trx_status(
                bank_trx_obj, json.loads(response.json())
            )
            return Response(
                BankTransactionResponseModelSerializer(new_bank_trx_obj).data
            )
        except (HTTPError, ConnectionError, Exception) as e:
            ACH_GET_TRX_STATUS_LOGGER.debug(
                _(
                    f"[message] [ACH EXCEPTION] [{bank_trx_obj.user_created}] [bank_trx_id ==> {str(bank_trx_obj.transaction_id)}] -- {e.args}"
                )
            )
            return Response(BankTransactionResponseModelSerializer(bank_trx_obj).data)
    ...
    def update_manual_batch_transactions(trn_data):
        for trn in trn_data:
            try:
                bank_trx_obj = BankTransaction.objects.get(
                    transaction_id=trn.get("transaction_id")
                )
                ACH_GET_TRX_STATUS_LOGGER.debug(
                    _(
                        f"[message] [Bank Manual batch update] [{bank_trx_obj.user_created}] [bank_trx_id ==> {str(bank_trx_obj.transaction_id)}"
                    )
                )
                (
                    status,
                    status_desc,
                ) = BankTransactionsChannel.map_manual_batch_status_code_desc(
                    trn.get("status")
                )
                if (
                    ...
                    new_trn_obj = (
                        BankTransactionsChannel.create_new_trx_out_of_passed_one(
                            bank_trx_obj
                        )
                    )
                    ...
                    if status in ["000100", "000005"]:
                        balance_before = (
                            new_trn_obj.user_created.root.budget.get_current_balance()
                        )
                        balance_after = new_trn_obj.user_created.root.budget.return_disbursed_amount_for_cancelled_trx(
                            new_trn_obj.amount
                        )
                    ...
                    for single_trn in BankTransaction.objects.filter(
                        ...
                        single_trn.bank_end_to_end_identifier = trn.get(
                    ...
                    if bank_trx_obj.user_created.root.root.callback_url:
                        ...
                        callback_payload = {
                            "transaction_id": str(
                                new_trn_obj.parent_transaction.transaction_id
                            ),
                            "issuer": "bank_card",
                            "amount": str(new_trn_obj.amount),
                            "bank_card_number": str(
                                new_trn_obj.creditor_account_number
                            ),
                            "full_name": str(new_trn_obj.creditor_name),
                            "bank_code": str(new_trn_obj.creditor_bank),
                            "bank_transaction_type": str(
                                bank_trx_obj.get_transaction_type
                            ),
                            "disbursement_status": str(
                                new_trn_obj.status_choice_verbose
                            ),
                            "status_code": str(new_trn_obj.transaction_status_code),
                            "status_description": str(
                                new_trn_obj.transaction_status_description
                            ),
                            "client_transaction_reference": str(
                                new_trn_obj.client_transaction_reference
                            ),
                            "created_at": new_trn_obj.parent_transaction.created_at.strftime(
                                "%Y-%m-%d %H:%M:%S.%f"
                            ),
                            "updated_at": new_trn_obj.updated_at.strftime(
                                "%Y-%m-%d %H:%M:%S.%f"
                            ),
                        }
                        response = requests.post(callback_url, json=callback_payload)
                ...
                ACH_GET_TRX_STATUS_LOGGER.debug(
                    _(
                        f"[message] [ACH EXCEPTION] [bank_trx_id ==> {trn.get('bank_transaction_id')}] -- {e.args}"
                    )
                )
