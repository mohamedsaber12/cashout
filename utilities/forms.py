    ...
    def clean(self):
        ...
        try:
            ...
            if amount_being_added:
                ...
                cleaned_data["current_balance"] = (
                    self.budget_object.current_balance + amount_being_added
                )
                cleaned_data["created_by"] = self.superadmin_user
        except ValueError:
            self.add_error(
                'new_amount',
                _('New amount must be a valid integer, please check and try again.'),
            )
...
class BudgetAdminModelForm(forms.ModelForm):
    ...
    fx_rate = forms.FloatField(
        required=False,
        validators=[
            MinValueValidator(round(Decimal(0.1), 2)),
            MaxValueValidator((round(Decimal(100), 2))),
        ],
        help_text=_(
            'If the amount charged by USD please add the fx rate if not leave empty '
        ),
...
class IncreaseBalanceRequestForm(forms.Form):
    ...
    username = forms.CharField(
        ...
        required=False,
        widget=forms.TextInput(
            attrs={'placeholder': _('Accept Username'), 'class': 'form-control'}
        ),
    )
    from_bank = forms.CharField(
        ...
        required=False,
        widget=forms.TextInput(
            attrs={'placeholder': _('Bank Name'), 'class': 'form-control'}
        ),
    )
    ...
    from_account_number = forms.CharField(
        ...
        max_length=34,
        widget=forms.TextInput(
            attrs={'placeholder': _('From Account Number'), 'class': 'form-control'}
        ),
    )
    to_account_number = forms.CharField(
        ...
        max_length=34,
        widget=forms.TextInput(
            attrs={'placeholder': _('To Account Number'), 'class': 'form-control'}
        ),
    )
    from_account_name = forms.CharField(
        ...
        required=False,
        widget=forms.TextInput(
            attrs={'placeholder': _('From Account Name'), 'class': 'form-control'}
        ),
    )
    to_account_name = forms.CharField(
        ...
        required=False,
        widget=forms.TextInput(
            attrs={'placeholder': _('To Account Name'), 'class': 'form-control'}
        ),
    )
    from_date = forms.DateField(
        ...
        widget=forms.DateInput(
            attrs={'placeholder': _('Enter Date'), 'class': 'form-control'}
        ),
    )
...
class MerchantLimitUpdateForm(forms.ModelForm):
    merchant_limit = forms.DecimalField(
        required=True,
        widget=forms.TextInput(
            attrs={'placeholder': _('Your Limit'), 'class': 'form-control m_limit'}
        ),
    )
